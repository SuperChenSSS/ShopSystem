...
def Match_User():
	...
	cursor = c.execute('''SELECT PASSWORD FROM USER 
			WHERE NAME="{NAME}"'''.format(NAME=username))
	for cur in cursor:
		if str(cur[0]) == password:
...
def query_All_SQL(attributes="",value=""):
	...
	if attributes == "" and value == "":
		cursor = c.execute("SELECT * FROM SHOP")
	elif attributes == "SUM":
		print("商品名称\t商品总价格")
		cursor = c.execute("SELECT GOODS_NAME,SUM(PRICE * AMOUNT) FROM SHOP GROUP BY GOODS_NAME")
		for row in cursor:
			print(row[0],end='\t')
			print(row[1])
		pass
		...
		print("经办人\t总数量")
		cursor = c.execute("SELECT PERSON_NAME,SUM(AMOUNT) FROM SHOP GROUP BY PERSON_NAME")
		for row in cursor:
			print(row[0],end='\t')
			print(row[1])
		pass
	else:
		cursor = c.execute("SELECT * from SHOP WHERE " + attributes + "=\"" + value + "\";")
